...
void ClaireAllocation::init() {
	...
	CHECKED_ALLOC(ClRes->hafStack, CL_INT*, maxHist) // address part of the stack
	...
	CHECKED_ALLOC(ClEnv->stack, int, maxStack) // eval stack
...
CL_INT startClaire(CL_INT argc, char *argv[]) {
	...
	if ERROR_IN {
		...
		claire_started(); //<sb> only once the comman line has been parsed (stdin)
		CL_exit(0);
	} else {
		c_handle.catchIt();
...
{CL_INT m;
  CL_INT cells = n / 4 + 4; //<sb> convert length to cells (+ 1 (4 bytes) for null char)
  if(cells < OPTIMIZE) cells = OPTIMIZE;
  m = newChunk(cells);
  unsigned *a = ADRTOPOIN(m);
...
CL_INT ClaireAllocation::freeChunk(CL_INT n) {
  unsigned *nl = ADRTOPOIN(n);//Cmemory[n];             /* real length of the allocated space */
...
{unsigned *l = ADRTOPOIN(n);
 CL_INT size = log2up(*l) - 1;
 unsigned z = (CL_UNSIGNED)n - CmemoryAdr;
...
CL_INT ClaireAllocation::mergeRight(CL_INT a, CL_INT b, CL_INT size) {
  unsigned *bb = ADRTOPOIN(b);
  if (entryList[size] == b)    // b is the first free of this size
      entryList[size] = bb[FOLLOW];
  else
     { unsigned previous = bb[PREVIOUS];
       unsigned follow = bb[FOLLOW];
...
CL_INT ClaireAllocation::mergeLeft(CL_INT a, CL_INT b, CL_INT size) {
  unsigned *aa = ADRTOPOIN(a);
  if (entryList[size] == a) // a is the first free cell 
     entryList[size] = (CL_INT)aa[FOLLOW];
  else
     { unsigned previous = aa[PREVIOUS];
       unsigned follow = aa[FOLLOW];
       ADRTOPOIN(previous)[FOLLOW] = follow;
       if (follow != NOTHING)
           ADRTOPOIN(follow)[PREVIOUS] = previous;}
  *aa *= 2;
...
{ if CLMEM(s)
   {unsigned x = POINTOADR(s) - 3; //<sb> -2 for buffer string
    freeChunk(x);}}
