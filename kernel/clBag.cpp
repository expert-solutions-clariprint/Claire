...
list * cast_I_list1(list *l, ClaireType *x)  { l->of = x; return l;}
set * cast_I_set1(set *l, ClaireType *x)  { l->of = x; return l;}
...
list *make_list_type1(ClaireType *t, CL_INT n)
{  if ( n < 0 ) n = 0;
   list *obj = list::make();
   CL_INT i;
   if (n > 0)
...
list *prealloc_list_type1(ClaireType *t, CL_INT n)
{  if ( n < 0 ) n = 32; 
   list *obj = list::make();
   OID *x = ClAlloc->makeContent(1);
   obj->content = x;
...
{CL_INT i, m = l->length;
    ...
    if (m <= n) l->length = 0;
        ...
        upmove(l->content + 1, l->content + 1 + n, m - n); //<sb> v3.3.33 
        l->length = m - n;}
   return l; }
...
bag *shrink_list (bag *l, CL_INT n)
{if (n < 0) Cerror(7,_oid_(l),n);
...
//<sb> allocate an empty list that is prealloc-able
// use this for storeable lists
set *prealloc_set_type1(ClaireType *t, CL_INT n)
{  if ( n < 0 ) n = 32; 
   set *obj = set::make();
   OID *x = ClAlloc->makeContent(1);
   obj->content = x;
...
#ifdef __LP64__
 CL_INT n = m; // ((t == Kernel._float) ? m + 1 : m);     // v3.2.44
#else
...
OID nth_get_array(OID *a, CL_INT n)
...
void nth_put_array(OID *a, CL_INT n, OID y)
