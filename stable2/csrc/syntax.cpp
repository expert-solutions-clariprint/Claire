...
{ GC_BIND;
  if (0 <= (CL_INT)ClEnv->debug_I)
   { (Language.LastCall->value= _oid_(c));
    put_table(Reader.DBline,_oid_(c),((OID)Reader.reader->nb_line));
    }
  if ((c->selector == Kernel.store) && 
      (c->args->length == 1))
   { list * l = GC_OBJECT(list,c->args);
    if (INHERIT(OWNER((*(l))[1]),Core._global_variable))
     ((*(l))[1]=GC_OID(_string_(make_string_symbol(OBJECT(symbol,(*Kernel.name)((*(l))[1]))))));
    }
    ...
    { CL_INT loop_handle = ClEnv->cHandle;
      ...
      { { OID  loc = (*(idents))[i];
          if (equal((*Kernel.nth)(loc,
            ((OID)1)),_string_(c->selector->name->name)) == CTRUE)
           { (*Kernel._inf_inf)(loc,
              ((OID)1));
...
{ GC_BIND;
  { Call *Result ;
    { Call * g0629UU;
      ...
      Result = DBregister_Call(g0629UU);
      }
    GC_UNBIND; POP_SIGNAL; return (Result);}
  }
...
{ GC_BIND;
  { OID Result = 0;
    if (y == _oid_(Reader.as))
     ...
     { if (INHERIT(OWNER(x),Core._global_variable))
       ...
       { if ((INHERIT(OWNER(z),Language._Call)) && (((OBJECT(Call,z)->selector == Kernel.add) || 
         ...
         { Call * V_CL0631;{ list * g0632UU;
            { OID v_bag;
              GC_ANY(g0632UU= list::empty(Kernel.emptySet));
              { if (OBJECT(Call,x)->args->length == 1)
                 v_bag = _oid_(OBJECT(Call,x)->selector);
                else v_bag = (*(OBJECT(Call,x)->args))[1];
                  GC_OID(v_bag);}
              ((list *) g0632UU)->addFast((OID)v_bag);
              { if (OBJECT(Call,x)->args->length == 1)
                 v_bag = (*(OBJECT(Call,x)->args))[1];
                else v_bag = (*(OBJECT(Call,x)->args))[2];
                  GC_OID(v_bag);}
              ((list *) g0632UU)->addFast((OID)v_bag);
              ((list *) g0632UU)->addFast((OID)GC_OID((*(OBJECT(bag,(*Core.args)(z))))[2]));}
            V_CL0631 = Call_I_property(OBJECT(property,(*Kernel.selector)(z)),g0632UU);
            }
          
          Result=_oid_(V_CL0631);}
        ...
        else Serror_integer1(164,"~S cannot be assigned with :=",list::alloc(1,x));
          }
                ...
                { Do * g0635 = _CL_obj; 
                  ...
                  { list * i_bag = list::empty(Kernel._any);
                    { CL_INT  i = 1;
                      ...
                      { CL_INT loop_handle = ClEnv->cHandle;
                        OID gc_local;
                        while (((CL_INT)i <= (CL_INT)g0630))
                        { GC_LOOP;
                          { OID  g0637UU;
                            { Assign * _CL_obj = ((Assign *) GC_OBJECT(Assign,new_object_class(Language._Assign)));
                              (_CL_obj->var = (*(l))[i]);
                              (_CL_obj->arg = _oid_(Call_I_property(Kernel.nth,GC_OBJECT(list,list::alloc(2,_oid_(v),((OID)i))))));
                              add_I_property(Kernel.instances,Language._Assign,11,_oid_(_CL_obj));
                              g0637UU = _oid_(_CL_obj);
                              }
                            i_bag->addFast((OID)g0637UU);
                            }
                          ++i;
                          GC_UNLOOP;POP_SIGNAL;}
                        }
                      }
                    g0636 = GC_OBJECT(list,i_bag);
                    }
                  (g0635->args = g0636);}
     ...
     Result = _oid_(Call_I_property(Kernel._Z,GC_OBJECT(list,list::alloc(2,x,z))));
    else { Call * V_CL0638;{ Call_star * g0639UU;
          ...
          V_CL0638 = DBregister_Call(g0639UU);
          }
        
        Result=_oid_(V_CL0638);}
      GC_UNBIND; POP_SIGNAL; return (Result);}
  }
...
{ GC_BIND;
  { OID Result = 0;
    { OID  _Zdef = GC_OID(nexts_I_meta_reader3(r,Reader.in,44));
       ...
       { Variable * v = GC_OBJECT(Variable,extract_variable_any(GC_OID(OBJECT(Assign,_Zdef)->var)));
        ...
        { { Let * _CL_obj = ((Let *) GC_OBJECT(Let,new_object_class(Language._Let)));
            ...
            { Let * g0640 = _CL_obj; 
              OID  g0641;
              if (firstc_meta_reader(r) == 44)
               g0641 = readlet_meta_reader(cnext_meta_reader(r),e);
              else g0641 = nexts_meta_reader(r,e);
                (g0640->arg = g0641);}
            ...
            { Let * g0644 = _CL_obj; 
              ...
              { Do * _CL_obj = ((Do *) GC_OBJECT(Do,new_object_class(Language._Do)));
                { Do * g0646 = _CL_obj; 
                  list * g0647;
                  { OID v_bag;
                    GC_ANY(g0647= list::empty(Kernel._any));
                    ((list *) g0647)->addFast((OID)_Zdef);
                    { { Let * _CL_obj = ((Let *) GC_OBJECT(Let,new_object_class(Language._Let)));
                        (_CL_obj->var = v2);
                        (_CL_obj->value = _Ze);
                        { Let * g0648 = _CL_obj; 
      ...
      else Serror_integer1(165,"~S is illegal after a let",list::alloc(1,_Zdef));
        }
    GC_UNBIND; POP_SIGNAL; return (Result);}
  }
     ...
     { if (firstc_meta_reader(r) == 44)
       Result = readlet_meta_reader(cnext_meta_reader(r),e);
        ...
        { { Let * _CL_obj = ((Let *) GC_OBJECT(Let,new_object_class(Language._Let)));
            ...
            (_CL_obj->arg = readlet_star_meta_reader(r,l,((CL_INT)n+(CL_INT)1),e));
            add_I_property(Kernel.instances,Language._Let,11,_oid_(_CL_obj));
            x = _CL_obj;
            }
          GC_OBJECT(Let,x);}
...
{ GC_BIND;
  { OID Result = 0;
    { OID  _Zdef = GC_OID(nexts_I_meta_reader3(r,Reader.in,44));
        ...
        { { When * _CL_obj = ((When *) GC_OBJECT(When,new_object_class(Language._When)));
            ...
            { When * g0650 = _CL_obj; 
              ...
              if (wcl_start_ask_meta_reader1(r,firstc_meta_reader(r)) == CTRUE)
               ...
               g0651 = CNULL;
              else g0651 = nexts_meta_reader(r,e);
                (g0650->other = g0651);}
      ...
      else Serror_integer1(165,"~S is illegal after a when",list::alloc(1,_Zdef));
        }
    GC_UNBIND; POP_SIGNAL; return (Result);}
  }
    ...
    { OID  _Za1 = GC_OID(nexte_meta_reader(r));
      ...
      { If * g0652 = _CL_obj; 
        ...
        { if (should_trace_ask_module1(Reader.it,3) == CTRUE)
           mtformat_module1(Reader.it,"readif firstc = ~S \n",3,list::alloc(1,_oid_(char_I_integer(firstc_meta_reader(r)))));
           ...
           g0653 = Kernel.cfalse;
              ...
              if (x == _oid_(Reader._cl_if))
               g0653 = readif_meta_reader(r,e);
              else if (keyword_ask_any(x) == CTRUE)
               g0653 = nextstruct_meta_reader(r,OBJECT(keyword,x),Reader.none);
              else g0653 = loopexp_meta_reader(r,x,Reader.none,CFALSE);
                }
            }
        (g0652->other = g0653);}
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  { OID Result = 0;
    { OID  _Zv = GC_OID(nexte_meta_reader(r));
      if (skipc_I_meta_reader(r) != ((OID)40))
       Serror_integer1(166,"Missing ( after case ~S",list::alloc(1,_Zv));
      { Case * _Zx;
        ...
        { CL_INT loop_handle = ClEnv->cHandle;
          ...
          { GC_LOOP;
            ...
            (_Zx->args = GC_OBJECT(list,GC_OBJECT(list,_Zx->args)->addFast((OID)_Zt))->addFast((OID)GC_OID(nexts_meta_reader(r,Reader.none))));
            if ((boolean_I_any(stop_ask_integer(firstc_meta_reader(r))) != CTRUE) && 
                (boolean_I_any(stop_ask_integer(skipc_meta_reader(r))) != CTRUE))
             Serror_integer1(167,"Missing ) or , after ~S",list::alloc(1,_oid_(_Zx)));
            GC_UNLOOP;POP_SIGNAL;}
          }
        next_meta_reader(r);
        if ((e != Reader.none) && 
         ...
         Serror_integer1(161,"Missing ~S after ~S",GC_OBJECT(list,list::alloc(2,_oid_(e),_oid_(_Zx))));
        Result = _oid_(_Zx);
        }
      }
    GC_UNBIND; POP_SIGNAL; return (Result);}
     ...
     { next_meta_reader(r);
      Result = _oid_(Kernel.emptySet);
        ...
        { OID  _Za2 = GC_OID(nexte_meta_reader(r));
          if (equal(_Za2,r->comma) == CTRUE)
           { set * V_CL0654;{ set * g0655UU;
              { set * u_bag = set::empty(Kernel.emptySet);
                ...
                g0655UU = GC_OBJECT(set,u_bag);
                }
              V_CL0654 = cast_I_set1(g0655UU,Kernel.emptySet);
              }
            
            Result=_oid_(V_CL0654);}
           ...
           { Variable * v = GC_OBJECT(Variable,extract_variable_any(_Za1));
            ...
            { Iteration * g0656 = _CL_obj; 
              OID  g0657;
              { list * _Zbind = GC_OBJECT(list,bind_I_meta_reader(r,v));
                OID  x;
                { if (equal(GC_OID(nexte_meta_reader(r)),GC_OID(Reader.OR->value)) != CTRUE)
                   Serror_integer1(168,"Missing | in selection",Kernel.nil);
           ...
           { Variable * v = GC_OBJECT(Variable,extract_variable_any(GC_OID(nexts_I_meta_reader1(r,Reader.in))));
            { OID  g0658UU;
              { Image * _CL_obj = ((Image *) GC_OBJECT(Image,new_object_class(Language._Image)));
                ...
                g0658UU = _oid_(_CL_obj);
                }
              Result = lexical_build_any(g0658UU,GC_OBJECT(list,list::alloc(1,_oid_(v))),0);
              }
            }
          ...
          else Serror_integer1(169,"Missing separation between ~S and ~S",GC_OBJECT(list,list::alloc(2,_Za1,_Za2)));
            }
        }
      GC_UNBIND; POP_SIGNAL; return (Result);}
  }
...
{ { OID Result = 0;
    if (INHERIT(OWNER(x),Kernel._unbound_symbol))
     Serror_integer1(170,"Cannot use ~S in a set constant",list::alloc(1,x));
    else if (INHERIT(OWNER(x),Language._Variable))
     Serror_integer1(170,"Cannot use a variable (~S) in a set constant",list::alloc(1,x));
    ...
    if (firstc_meta_reader(r) == e)
     { next_meta_reader(r);
      Result = _oid_(list::empty());
        ...
        else Serror_integer1(171,"Read the character ~S inside a sequence",list::alloc(1,_oid_(char_I_integer(firstc_meta_reader(r)))));
          }
      GC_UNBIND; POP_SIGNAL; return (Result);}
  }
...
{ GC_BIND;
  ...
  { OID Result = 0;
    if (equal(x,r->paren) == CTRUE)
     ...
     Serror_integer1(172,"The sequence ...~S must end with ~A",list::alloc(2,x,_oid_(char_I_integer(e))));
     ...
     { If * _Zi = GC_OBJECT(If,OBJECT(If,readif_meta_reader(r,e)));
      { OID  g0659UU;
        { if (boolean_I_any(eval_any(GC_OID(_Zi->test))) == CTRUE)
           g0659UU = _Zi->arg;
          else g0659UU = _Zi->other;
            GC_OID(g0659UU);}
        Result = readblock_meta_reader(r,g0659UU,e);
        }
      }
...
// The c++ function for: localize!(self:Definition) [NEW_ALLOC+BAG_UPDATE+SLOT_UPDATE+RETURN_ARG]
OID  localize_I_Definition1(Definition *self)
{ GC_BIND;
  { OID Result = 0;
    { list * idents = GC_OBJECT(list,OBJECT(list,Reader.IDENTS->value));
      CL_INT  i = idents->length;
      { CL_INT loop_handle = ClEnv->cHandle;
        ...
        { { OID  loc = (*(idents))[i];
            if (equal((*Kernel.nth)(loc,
              ((OID)1)),_string_(self->arg->name->name)) == CTRUE)
             { (*Kernel._inf_inf)(loc,
                ((OID)1));
              if (boolean_I_any(loc) == CTRUE)
               update_property(Kernel.location,
    ...
    { OID  l = GC_OID(nextseq_meta_reader(cnext_meta_reader(r),41));
      if (x == _oid_(Reader.PRINTF))
       ...
       { Return * _CL_obj = ((Return *) GC_OBJECT(Return,new_object_class(Language._Return)));
        { Return * g0663 = _CL_obj; 
          OID  g0664;
          if (boolean_I_any(l) == CTRUE)
           g0664 = (*Kernel.nth)(l,
            ((OID)1));
          else g0664 = Kernel.ctrue;
            (g0663->arg = g0664);}
        add_I_property(Kernel.instances,Language._Return,11,_oid_(_CL_obj));
        Result = _oid_(_CL_obj);
        }
      else if (INHERIT(OWNER(x),Kernel._class))
       { { ClaireBoolean * g0665I;
          { OID  g0666UU;
            { ClaireBoolean * V_CL0667;{ OID  g0668UU;
                { CL_INT loop_handle = ClEnv->cHandle;
                  OID gc_local;
                  ITERATE(y);
                  g0668UU= Kernel.cfalse;
                  bag *y_support;
                  y_support = GC_OBJECT(bag,enumerate_any(l));
                  for (START(y_support); NEXT(y);)
                  { GC_LOOP;
                    { ClaireBoolean * g0669I;
                      { OID  g0670UU;
                        if (INHERIT(OWNER(y),Language._Call))
                         { if (OBJECT(Call,y)->selector == Kernel._equal)
                           { ((*(GC_OBJECT(list,OBJECT(Call,y)->args)))[1]=_oid_(make_a_property_any(GC_OID((*(OBJECT(Call,y)->args))[1]))));
                            g0670UU = Kernel.ctrue;
                            }
                          else g0670UU = Kernel.cfalse;
                            }
                        else g0670UU = Kernel.cfalse;
                          g0669I = not_any(g0670UU);
                        }
                      
                      if (g0669I == CTRUE) { g0668UU = Kernel.ctrue;
                          ClEnv->cHandle = loop_handle;break;}
                        }
                    GC_UNLOOP; POP_SIGNAL;}
                  }
                V_CL0667 = not_any(g0668UU);
                }
              
              g0666UU=_oid_(V_CL0667);}
            g0665I = not_any(g0666UU);
            }
          
          if (g0665I == CTRUE) { list * l2 = OBJECT(ClaireClass,x)->params;
              CL_INT  n = ((INHERIT(l2->isa,Kernel._list)) ?
                l2->length :
                0 );
              if ((*Kernel.length)(l) == ((OID)n))
               { list * V_CL0671;{ list * i_bag = list::empty(Kernel.emptySet);
                  { CL_INT  i = 1;
                    CL_INT  g0660 = n;
                    { CL_INT loop_handle = ClEnv->cHandle;
                      OID gc_local;
                      while (((CL_INT)i <= (CL_INT)g0660))
                      { GC_LOOP;
                        { OID  g0672UU;
                          { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
                            ...
                            (_CL_obj->args = list::alloc(2,(*(l2))[i],GC_OID((*Kernel.nth)(l,
                              ((OID)i)))));
                            add_I_property(Kernel.instances,Language._Call,11,_oid_(_CL_obj));
                            g0672UU = _oid_(_CL_obj);
                            }
                          i_bag->addFast((OID)g0672UU);
                          }
                        ++i;
                        GC_UNLOOP;POP_SIGNAL;}
                      }
                    }
                  V_CL0671 = GC_OBJECT(list,i_bag);
                  }
                
                l=_oid_(V_CL0671);}
              else Serror_integer1(174,"Wrong instantiation list ~S(~S...",GC_OBJECT(list,list::alloc(2,x,GC_OID(_oid_(list::alloc(1,l))))));
                }
            }
        { Definition * g0673UU;
          ...
          Result = localize_I_Definition1(g0673UU);
          }
        }
      else { ClaireBoolean * g0674I;
        { ClaireBoolean *v_or;
          { v_or = inherit_ask_class(OWNER(x),Language._Variable);
            if (v_or == CTRUE) g0674I =CTRUE; 
            else { { OID  g0675UU;
                if (INHERIT(OWNER(x),Core._global_variable))
                 g0675UU = _oid_(OBJECT(global_variable,x)->range);
                else g0675UU = Kernel.cfalse;
                  v_or = boolean_I_any(g0675UU);
                }
              if (v_or == CTRUE) g0674I =CTRUE; 
              else g0674I = CFALSE;}
            }
        ...
        if (g0674I == CTRUE) Result = _oid_(Call_I_property(Core.call,cons_any(x,GC_OBJECT(list,((boolean_I_any(l) == CTRUE) ?
            OBJECT(list,l) :
    ...
    if (equal(y,Reader.triangle->value) == CTRUE)
     ...
     { ClaireBoolean * table_ask = ((INHERIT(OWNER(x),Language._Call)) ?
      ...
      if ((table_ask == CTRUE) ? (w == _oid_(Reader.L__equal)) : ((equal(w,Reader.arrow->value) == CTRUE) || 
       ...
       ;else Serror_integer1(149,"Wrong keyword (~S) after ~S",list::alloc(2,w,z));
     ...
     { if (INHERIT(OWNER(x),Language._Call))
       { OID  ru = GC_OID(nexte_meta_reader(r));
        ...
        (_CL_obj->arg = z);
        { Defrule * g0676 = _CL_obj; 
          OID  g0677;
          if (firstc_meta_reader(r) == 41)
           { next_meta_reader(r);
            g0677 = Core.nil->value;
            }
          else g0677 = readblock_meta_reader(r,GC_OID(nexte_meta_reader(r)),41);
            (g0676->body = g0677);}
     ...
     { (r->last_arrow = equal(y,_oid_(Reader._equal_sup)));
      if (should_trace_ask_module1(Reader.it,3) == CTRUE)
       mtformat_module1(Reader.it,"---- note: ~S - method's range is assumed to be void \n",3,list::alloc(1,x));
    ...
    { OID  n = skipc_meta_reader(r);
      ...
      { { Defmethod * _CL_obj = ((Defmethod *) GC_OBJECT(Defmethod,new_object_class(Language._Defmethod)));
          update_property(Kernel.arg,
            ...
            x);
          (_CL_obj->set_arg = y);
          { Defmethod * g0678 = _CL_obj; 
            OID  g0679;
            if (z == _oid_(Reader._cl_let))
             g0679 = readlet_meta_reader(r,Reader.None);
            else g0679 = z;
              (g0678->body = g0679);}
  ...
  { OID Result = 0;
    if (INHERIT(OWNER(x),Language._Variable))
     { Defobj * _CL_obj = ((Defobj *) GC_OBJECT(Defobj,new_object_class(Language._Defobj)));
      (_CL_obj->ident = OBJECT(Variable,x)->pname);
      (_CL_obj->arg = Core._global_variable);
      { Definition * g0680 = _CL_obj; 
        list * g0681;
        { OID v_bag;
          GC_ANY(g0681= list::empty(Kernel.emptySet));
          { { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
              (_CL_obj->selector = Kernel._equal);
     ...
     { OID  ru = GC_OID(nexte_meta_reader(r));
      ...
      (_CL_obj->args = OBJECT(Call,x)->args);
      (_CL_obj->arg = z);
      { Defrule * g0682 = _CL_obj; 
        OID  g0683;
        if (firstc_meta_reader(r) == 41)
         { next_meta_reader(r);
          g0683 = Core.nil->value;
          }
        else g0683 = readblock_meta_reader(r,GC_OID(nexte_meta_reader(r)),41);
          (g0682->body = g0683);}
      add_I_property(Kernel.instances,Language._Defrule,11,_oid_(_CL_obj));
      Result = _oid_(_CL_obj);
      }
        ...
        if ((INHERIT(OWNER(x),Core._global_variable)) && (y == CNULL))
         ...
         { Defobj * _CL_obj = ((Defobj *) GC_OBJECT(Defobj,new_object_class(Language._Defobj)));
          ...
          Result = _oid_(_CL_obj);
            ...
            (_CL_obj->ident = extract_symbol_any(x));
            (_CL_obj->arg = Core._global_variable);
            { Definition * g0684 = _CL_obj; 
              list * g0685;
              { OID v_bag;
                GC_ANY(g0685= list::empty(Kernel.emptySet));
                { { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
                    (_CL_obj->selector = Kernel._equal);
    ...
    { OID  c = GC_OID(verify_any(_oid_(Kernel._class),GC_OID(read_ident_meta_reader1(r)),_oid_(Language._Defclass)));
            ...
            { ClaireBoolean * g0686I;
              { OID  g0687UU;
                if (INHERIT(OWNER(y1),Language._Call))
                 g0687UU = _oid_(((OBJECT(Call,y1)->selector == Kernel._equal) ? ((INHERIT(OWNER((*(OBJECT(Call,y1)->args))[1]),Language._Vardef)) ? CTRUE: CFALSE): CFALSE));
                else if (INHERIT(OWNER(y1),Language._Vardef))
                 g0687UU = Kernel.ctrue;
                else g0687UU = Kernel.cfalse;
                  g0686I = not_any(g0687UU);
                }
              
              if (g0686I == CTRUE) Serror_integer1(175,"Wrong form ~S in ~S(~S)",list::alloc(3,y1,
                  c,
                  l));
                }
            }
      ...
      if ((INHERIT(OWNER(x),Language._Call)) && (OBJECT(Call,x)->selector == Kernel.nth))
       { list * l = GC_OBJECT(list,OBJECT(Call,x)->args);
        ...
        if ((old_ask == CTRUE) && 
          (skipc_meta_reader(r) != ((OID)93)))
       Serror_integer1(176,"Missing ] after ~S ",GC_OBJECT(list,list::alloc(1,_oid_(y))));
