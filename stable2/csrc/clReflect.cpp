...
//<sb> add a CLSMALL like optimized inheritance check.
// For each instance of class is associated a table that
// contain the inherit flags for another class in the
// system. The bit at the index of the tested class
// tell whether that class is an ancestor

unsigned inherit_map_size = 0;
unsigned inherit_class_index = 0;

void ClaireClass::bootstrap_inherit_map() {
	inherit_map_size += 64;
	Kernel._void->inherit_map = (unsigned*)malloc(8);
	Kernel._void->inherit_mask = 1;
	Kernel._void->inherit_base_index = 0;
	*Kernel._void->inherit_map = 1;
	inherit_class_index = 1;
}

void ClaireClass::insert_inherit_map() {
	if (inherit_class_index + 1 >= inherit_map_size) {
		unsigned old = inherit_map_size / 32;
		inherit_map_size += 64;

		CL_INT i = 1;
		list *l = Kernel._class->instances;
		for(;i < l->length;i++) {
			ClaireClass *c = OBJECT(ClaireClass, (*l)[i]);
			c->inherit_map = (unsigned*)realloc(c->inherit_map, inherit_map_size / 4);
			c->inherit_map[old] = 0;
			c->inherit_map[old+1] = 0;
		}
	}

	inherit_class_index += 1;

	inherit_base_index = inherit_class_index / 32;
	inherit_mask = 1ll << (inherit_class_index % 32);

	inherit_map = (unsigned*)malloc(inherit_map_size / 4);
	memset(inherit_map, 0, inherit_map_size / 4);
	
	CL_INT i = 1;
	for(;i <= ancestors->length;i++) {
		ClaireClass *anc = OBJECT(ClaireClass, (*ancestors)[i]);
		inherit_map[anc->inherit_base_index] |= anc->inherit_mask;
	}
}
...
{method *m = (method *) Kernel._method->instantiate();
   ...
   m->evaluate = f;
   m->status = sta;
   m->compiled_variables = _oid_(CFALSE);
   m->location = Kernel.nil;
