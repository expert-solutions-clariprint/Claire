...
int PortObject::gets(void* data, CL_INT len) {
	return Core.read_port->fcall((CL_INT)this, (CL_INT)data, len);
}

int PortObject::eof_ask() {
	return Core.eof_port_ask->fcall((CL_INT)this) == Kernel.ctrue;
}
...
tuple *freadline_port6(PortObject* p, bag *seps, ClaireBoolean *sensitive, ClaireChar *escape) {
	if (Core.eof_port_ask->fcall((CL_INT)p) == (CL_INT)CTRUE) {
		...
		char *sep;
		char *travel = databuf;
		int isbuffer = p->isa == Core._buffer;
		int isblob = p->isa == Core._blob;
...
int unsensitive_match(char *travel, CL_INT seplen, char *sep) {
	int m = 0;
...
// ex: ascii code 1 is printed '\0d1'
