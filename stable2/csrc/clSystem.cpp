...
//<sb> time mode 0:local - 1:GMT
int time_mode = 0;

ClaireBoolean* timemode_boolean(ClaireBoolean *b) {
	int om = time_mode;
	if(b == CTRUE) time_mode = 1;
	else time_mode = 0;
	return om ? CTRUE : CFALSE;
}

//<sb> sleep until ms has elapsed
void sleep_integer(int ms) {
#ifdef CLPC
	Sleep(ms);
#else
	timeval tv;
...
void mktime_error(struct tm* atm) {
	...
	int dst_error;
	atm->tm_hour++;
	if(time_mode) dst_error = my_timegm(atm) != -1;
	else dst_error = mktime(atm) != -1;
	if(dst_error) {
		atm->tm_hour--;
		atm->tm_hour--;
...
int elapsed_float(double t) {
	return _integer_((CL_INT)(1000.0 * (timer_I_void() - t)));
}

int elapsed_float_(OID t) {
	return elapsed_float(float_v(t));
}

char* setlocale_integer(int cat, char *loc) {
	int category;
...
char* getlocale_integer(int cat) {
	int category;
	if(cat < 0 || cat > 5) Cerror(68,_integer_(cat),0);
	switch(cat) {
		case 0: category = LC_ALL; break;
...
double make_date_integer(int D, CL_INT M, CL_INT Y, CL_INT h, CL_INT m, CL_INT s) {
	struct tm atm;
	atm.tm_sec = s;
	atm.tm_min = m;
	atm.tm_hour = h;
	...
	time_t t;
	if(time_mode) t = my_timegm(&atm);
	else t = mktime(&atm);
	if(t == -1) mktime_error(&atm);
	return (double)t;
}

OID make_date_integer_(int D, CL_INT M, CL_INT Y, CL_INT h, CL_INT m, CL_INT s) {
...
double make_date_string(char *self) {
    ...
    if (sscanf(self, "%d-%d-%d %d:%d:%d", &y, &m, &d, &H, &M, &S) == 6) {}
    else if (sscanf(self, "%d-%d-%d %d:%d", &y, &m, &d, &H, &M) == 5) {S = 0;}
    else if (sscanf(self, "%d/%d/%d %d:%d:%d", &d, &m, &y, &H, &M, &S) == 6) {}
    else if (sscanf(self, "%d/%d/%d %d:%d", &d, &m, &y, &H, &M) == 5) {S = 0;}
		...
		if(time_mode) atm = gmtime(&t);
		else atm = localtime(&t);
...
double make_time_integer(int h, CL_INT m, CL_INT s) {
	struct tm atm; // set to January 1, 1970
	atm.tm_isdst = 0;
	atm.tm_year = 70;
	atm.tm_mon = 0;
...
OID make_time_integer_(int h, CL_INT m, CL_INT s) {
...
char *strftime_string(char* format, double t) {
	ClEnv->bufferStart();
	int maxlen = 1024 + LENGTH_STRING(format);
	ClEnv->pushAttempt(maxlen);
	struct tm *atm;
	time_t long_time = (time_t)t;	
    //atm = localtime(&long_time);
    if(time_mode) atm = gmtime(&long_time);
	else atm = localtime(&long_time);
    if(atm == 0) Cerror(70,0,0);
...
#if !defined(CLPC)
static inline CL_INT process_run(int p) {
	int res = 0;
	#ifdef HAVE_GETSID
	res = (CL_INT)getsid(p);
	#else
...
static inline void ensure_no_zombie(int process) {
	int status;
	(CL_INT *)process,  &status, WNOHANG|WUNTRACED);
}
#endif

//<sb> kill (brutal if needed) the given process
void kill_integer1(int process) {
#ifdef CLPC
...
ClaireBoolean* run_ask_integer(int pr) {
...
ClaireBoolean* wait_integer1(int pr, CL_INT ms) {
#ifdef CLPC
	HANDLE h = OpenProcess(SYNCHRONIZE, 0, pr);
	if(h) {
		int res = WaitForSingleObject(h, ms) == WAIT_OBJECT_0;
		CloseHandle(h);
...
tuple *waitpid_status_integer(int process, ClaireBoolean* block) {
#if defined(CLPC)
    ...
    pid_t wpid;
    tuple *result;
    // (CL_INT *)) returns 0 if the child process is still running, or the 
    // process id if it stopped.  It can also return -1 in case of error.
    if (block == CTRUE)
    	wpid = (CL_INT *)(pid_t)process, &status, WUNTRACED);
    else wpid = (CL_INT *)(pid_t)process, &status, WNOHANG|WUNTRACED);
...
tuple *waitpid_status_integer_(int process, ClaireBoolean* block) {
...
#ifdef CLPC
int pocess_I_string(char* self, bag* args) {
	PROCESS_INFORMATION pi;
	STARTUPINFO si;
	int created;
	int flag = CREATE_NEW_PROCESS_GROUP;
...
{
	int lto = LENGTH_STRING(to);
	if(lto == 0) return "";
	int lsrc = LENGTH_STRING(src);
	if(lto > lsrc) return copy_string(src);
	ClEnv->bufferStart();
	int lby = LENGTH_STRING(by);
	int i = 0; CL_INT l = 0;
	for(;i < lsrc - lto + 1;i++)
	{
		int same = 1; CL_INT j = 0;
		for(;j < lto;j++)
			if(to[j] != src[i + j])
				{same = 0; break;}
		if(same)
...
int occurrence_string(char* in, char* to)
{
	int lin = LENGTH_STRING(in);
	int lto = LENGTH_STRING(to);
...
int find_string(char* src, char* to, CL_INT from)
{
	if(from <= 0) return 0;
	int lsrc = LENGTH_STRING(src);
	int lto = LENGTH_STRING(to);
...
int rfind_string(char* src, char* to, CL_INT from)
{
...
int  find_string2(char *src,char *to)
...
int  rfind_string2(char *src,char *to)
...
#define EXPLODE_ADD(l, s, j) { \
	char *a = ClAlloc->makeString(j); \
	if(j) memcpy(a,s,j); \
	GC__ANY(l = l->addFast(GC_OID(_string_(a))), 1); }

static inline list *explode_char_string(char *self, char sep) {
	GC_RESERVE(1);
  	list * l = list::empty(Kernel._string);
	...
	{ GC_LOOP;
	  ...
	  //GC__ANY(l=l->addFast(GC_OID(_string_(substring_string(s2,1,s1 - s2 + 1)))), 1);
		s2 = s1+2;
		GC_UNLOOP;}
	if(s2 < end)
		EXPLODE_ADD(l, s2, end - s2 + 1)
		//GC__ANY(l=l->addFast(GC_OID(_string_(substring_string(s2,1,end - s2 + 1)))), 1);
	GC_UNBIND; return l;
}
...
static void encodeblock(unsigned char in[3], unsigned char out[4], CL_INT len) {
    out[0] = cb64[ in[0] >> 2 ];
...
void encode64_port(PortObject *infile, PortObject *outfile, CL_INT linesize) {
    ...
    while(!infile->eof_ask()) {
        len = 0;
        for(i = 0; i < 3; i++) {
            in[i] = (unsigned char) infile->get();
            if( !infile->eof_ask()) len++;
            else {in[i] = 0;}}
        if(len) {
            encodeblock( in, out, len );
            for( i = 0; i < 4; i++ )
                outfile->put((char)out[i]);
            blocksout++;}
        if(blocksout >= (linesize/4) || infile->eof_ask()) {
            if(blocksout) outfile->puts("\r\n",2);
            blocksout = 0;}}}
...
void decode64_port(PortObject *infile, PortObject *outfile) {
    ...
    CL_INT i, len;
    while(!infile->eof_ask()) {
        for(len = 0, i = 0; i < 4 && !infile->eof_ask(); i++) {
            v = 0;
            while(!infile->eof_ask() && v == 0) {
