/***** CLAIRE Compilation of file ./meta/types.cl 
         [version 3.3.36 / safety 5] Thu Nov 17 17:12:34 2005 *****/
...
{ { ClaireBoolean *Result ;
    if (Kernel._set == self->isa)
     ...
     { CL_INT  n = CLREAD(ClaireClass,self,open);
      Result = ((0 <= (CL_INT)n) ? (((CL_INT)n <= 2) ? ((self != Kernel._any) ? CTRUE: CFALSE): CFALSE): CFALSE);
      }
    else Result = CFALSE;
...
{ { ClaireBoolean *Result ;
    { OID  g0109UU;
      { CL_INT loop_handle = ClEnv->cHandle;
        ITERATE(x);
        g0109UU= Kernel.cfalse;
        for (START(self); NEXT(x);)
        if ((*Core.finite_ask)(x) != Kernel.ctrue)
         { g0109UU = Kernel.ctrue;
          ClEnv->cHandle = loop_handle;break;}
        }
      Result = not_any(g0109UU);
      }
    POP_SIGNAL; return (Result);}
  }
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  { OID Result = 0;
    { list * l = GC_OBJECT(list,self->args);
      { CL_INT  i = 1;
        CL_INT  g0110 = l->length;
        { CL_INT loop_handle = ClEnv->cHandle;
          OID gc_local;
          while (((CL_INT)i <= (CL_INT)g0110))
          { GC_LOOP;
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  if (should_trace_ask_module1(Core.it,5) == CTRUE)
   mtformat_module1(Core.it,"apply[@] ~S to ~S \n",5,list::alloc(2,_oid_(self),y));
  else ;{ CL_INT  i = 1;
    CL_INT  g0111 = l->length;
    { CL_INT loop_handle = ClEnv->cHandle;
      OID gc_local;
      while (((CL_INT)i <= (CL_INT)g0111))
      { GC_LOOP;
...
{ { CL_INT Result = 0;
    { CL_INT  n = 0;
      { CL_INT loop_handle = ClEnv->cHandle;
        ...
        n= ((CL_INT)n+(CL_INT)OBJECT(ClaireClass,x)->instances->length);
        }
      Result = n;
      }
    POP_SIGNAL; return (Result);}
...
{ GC_BIND;
  { CL_INT Result = 0;
    Result = (((INHERIT(x->t1->isa,Core._Interval)) || 
      ...
      ((CL_INT)(Core.size->fcall(((CL_INT) OBJECT(ClaireObject,GC_OID(_oid_(x->t1))))))+(CL_INT)(Core.size->fcall(((CL_INT) OBJECT(ClaireObject,GC_OID(_oid_(x->t2))))))) :
...
CL_INT  size_subtype(subtype *x)
{ GC_BIND;
  { CL_INT Result = 0;
    if (x->arg == Kernel._set)
     Result = exp2_integer(Core.size->fcall(((CL_INT) OBJECT(ClaireObject,GC_OID(_oid_(x->t1))))));
    else close_exception(((general_error *) (*Core._general_error)(_string_("[178] cannot enumerate ~S"),
        _oid_(list::alloc(1,_oid_(x))))));
      GC_UNBIND; POP_SIGNAL; return (Result);}
  }
      ...
      if (boolean_I_any(_oid_(l)) != CTRUE)
       Result = set::alloc(Kernel.emptySet,1,_oid_(Kernel.emptySet));
          ...
          { { set * y_bag = set::empty(Kernel.emptySet);
            ...
            GC_OBJECT(set,l1);}
          { CL_INT  n = 2;
            CL_INT  g0112 = l->length;
            { CL_INT loop_handle = ClEnv->cHandle;
              OID gc_local;
              while (((CL_INT)n <= (CL_INT)g0112))
              { GC_LOOP;
...
{ { CL_INT Result = 0;
    if (boolean_I_any(_oid_(l)) != CTRUE)
     Result = 1;
    else { CL_INT  m = (*Core.size)((*(l))[1]);
        { CL_INT  n = 2;
          CL_INT  g0113 = l->length;
          { CL_INT loop_handle = ClEnv->cHandle;
            OID gc_local;
            while (((CL_INT)n <= (CL_INT)g0113))
            { m= ((CL_INT)m*(CL_INT)((*Core.size)((*(l))[n])));
              ++n;
              POP_SIGNAL;}
            }
          }
    ...
    { ClaireObject *V_CC ;
      if (INHERIT(y->isa,Core._Union))
       ...
       { V_CC = ((INHERIT(OWNER(x),Kernel._integer)) ?
          (((CL_INT)CLREAD(Interval,y,arg1) <= x) ? ((x <= (CL_INT)CLREAD(Interval,y,arg2)) ? CTRUE: CFALSE): CFALSE) :
          CFALSE );
        }
    ...
    if ((x->open == ClEnv->ABSTRACT) && 
     ...
     { list * g0117UU;
      { { CL_INT loop_handle = ClEnv->cHandle;
          ...
          v_list = x->subclass;
           g0117UU = v_list->clone();
          for (CLcount= 1; CLcount <= v_list->length; CLcount++)
          { z = (*(v_list))[CLcount];
            v_val = (*Core.glb)(z,
              _oid_(y));
            
            (*((list *) g0117UU))[CLcount] = v_val;}
          }
        GC_OBJECT(list,g0117UU);}
      Result = Uall_list(g0117UU);
      }
        ...
        { GC_LOOP;
          { ClaireType * t = GC_OBJECT(ClaireType,OBJECT(ClaireType,(*Core.glb)(GC_OID(_oid_(_at_type(x,OBJECT(property,p)))),
            ...
            if (equal(_oid_(t),_oid_(Kernel.emptySet)) != CTRUE)
             GC__ANY(l = l->addFast((OID)_oid_(t)), 6);
       ...
       { OID  g0121UU;
        { CL_INT loop_handle = ClEnv->cHandle;
          ...
          if (_inf_equalt_class(OBJECT(ClaireClass,c),y) != CTRUE)
           { g0121UU = Kernel.ctrue;
            ClEnv->cHandle = loop_handle;break;}
          else ;}
        Result = not_any(g0121UU);
        }
...
{ GC_BIND;
  { ClaireBoolean *Result ;
    { OID  itest;
      { { OID  j_some = CNULL;
          { CL_INT  j = 1;
            CL_INT  g0124 = x->length;
            { CL_INT loop_handle = ClEnv->cHandle;
              while (((CL_INT)j <= (CL_INT)g0124))
       ...
       { ClaireBoolean *v_and;
        { v_and = ((x->length == ((bag *) y)->length) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { CL_INT  i = 1;
                CL_INT  g0123 = x->length;
                { CL_INT loop_handle = ClEnv->cHandle;
                  g0125UU= _oid_(CFALSE);
                  while (((CL_INT)i <= (CL_INT)g0123))
     ...
     { ClaireBoolean *v_and;
      { v_and = OBJECT(ClaireBoolean,_oid_((ClaireObject *) Core._inf_equalt->fcall(((CL_INT) x),((CL_INT) CLREAD(Param,y,arg)))));
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { CL_INT  n = 1;
              CL_INT  g0126 = CLREAD(Param,y,params)->length;
              { CL_INT loop_handle = ClEnv->cHandle;
                OID gc_local;
                g0127UU= _oid_(CFALSE);
                while (((CL_INT)n <= (CL_INT)g0126))
                { GC_LOOP;
     ...
     { list * g0128UU;
      { { CL_INT loop_handle = ClEnv->cHandle;
          ...
          { y = (*(v_list))[CLcount];
            ...
            (*((list *) g0128UU))[CLcount] = v_val;}
          }
        GC_OBJECT(list,g0128UU);}
      Result = Uall_list(g0128UU);
      }
    else Result = Kernel.emptySet;
      GC_UNBIND; POP_SIGNAL; return (Result);}
  }
    ...
    if (INHERIT(x->isa,Kernel._class))
     ...
     { list * g0129UU;
      { { CL_INT loop_handle = ClEnv->cHandle;
          ...
          OID y; CL_INT CLcount;
          v_list = ((bag *) x);
           g0129UU = v_list->clone();
          for (CLcount= 1; CLcount <= v_list->length; CLcount++)
          { y = (*(v_list))[CLcount];
            v_val = _oid_(set::alloc(1,GC_OID(funcall_property(p,y))));
            
            (*((list *) g0129UU))[CLcount] = v_val;}
          }
        GC_OBJECT(list,g0129UU);}
      Result = Uall_list(g0129UU);
      }
...
{ { CL_INT Result = 0;
    { CL_INT  n = 0;
      { CL_INT loop_handle = ClEnv->cHandle;
        ...
        if (((CL_INT)y <= 29) && 
            (0 <= (CL_INT)y))
         n= ((CL_INT)n+(CL_INT)exp2_integer(y));
        }
      Result = n;
      }
    POP_SIGNAL; return (Result);}
  ...
  { ClaireType *Result ;
    if (Kernel._set == xt1->isa)
     ...
     { if (INHERIT(xt2->isa,Core._Interval))
       { if (p == Core._plus)
         Result = _dot_dot_integer(((CL_INT)CLREAD(Interval,xt1,arg1)+(CL_INT)CLREAD(Interval,xt2,arg1)),((CL_INT)CLREAD(Interval,xt1,arg2)+(CL_INT)CLREAD(Interval,xt2,arg2)));
        else if (p == Kernel._dash)
         Result = _dot_dot_integer(((CL_INT)CLREAD(Interval,xt1,arg1)-(CL_INT)CLREAD(Interval,xt2,arg2)),((CL_INT)CLREAD(Interval,xt1,arg2)-(CL_INT)CLREAD(Interval,xt2,arg1)));
...
ClaireType * append_type_type1(ClaireType *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
...
ClaireType * cast_I_type_type1(ClaireType *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
...
ClaireType * prealloc_set_type_type1(ClaireType *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
    if (unique_ask_type(x) == CTRUE)
...
ClaireType * prealloc_list_type_type1(ClaireType *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
    if (unique_ask_type(x) == CTRUE)
...
{ { OID Result = 0;
    ...
    else { OID  V_CL0131;close_exception(((general_error *) (*Core._general_error)(_string_("[180] nth[~S] out of scope for ~S"),
          _oid_(list::alloc(2,((OID)x),_array_(self))))));
        
        Result=_void_(V_CL0131);}
      POP_SIGNAL; return (Result);}
  }
...
ClaireType * make_list_type_type1(ClaireType *x,ClaireType *y)
{ GC_BIND;
  { ClaireType *Result ;
    if (unique_ask_type(x) == CTRUE)
     Result = nth_class1(Kernel._list,GC_OBJECT(ClaireType,OBJECT(ClaireType,the_type(x))));
...
void  set_user_string1(char *username)
{ 
		#ifndef CLPC
		#ifdef HAVE_PWD_H
	;
