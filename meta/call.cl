...
claire/Call_wcl[selector,args] <: Call()
claire/Princ_wcl[selector,args] <: Call()

claire/echo :: property(open = 3)
claire/self_html :: property(open = 3)

self_print(self:Princ_wcl) : void -> printf("?>~A<?", self.args[1])
self_print(self:Call_wcl) : void ->
	(case self.selector
		({echo} princ("= "),
		{self_html} princ("== "),
		any printf("~A ", self.selector.name.name)),
	print(self.args[1]))
...
claire/breakpoint :: property()

self_eval(self:Call) : any
 -> (if (system.Kernel/debug! != -1) mClaire/push!(self),
       ...
       (if (system.Core/debug! >= 0) LastCall := self,
       	(for x in self.args mClaire/push!(eval(x)),
			let rx := eval_message(p, Core/find_which(p, start,
								 ...
								 start, true) in
				(if (system.Core/debug! >= 0) LastCall := self,
				if (system.Kernel/debug! != -1) mClaire/set_index(start - 1),
			   	rx))))


self_eval(self:Call+) : any
 -> let back_index := Core/index!(),
		back_debug := system.Kernel/debug!
	in (if (system.Kernel/debug! != -1)
			let d! := system.Kernel/debug!,
				st := (1 + Core/index!())
			in (Core/push!(self),
				Core/push!(self.selector),
				Core/push!(self.args[1]),
		...
		let p := self.selector,
	       ...
	       (if not(owner(s) = slot)
	        ...
	        else (preread(p, x), //<sb> v3.3.37
				let z := slot_get(x, (s as slot).index, (s as slot).mClaire/srange) in
	               (if (known?(z) | z % s.range)
	                   let n := system.Core/trace!,
	                   		d := system.Kernel/debug! in
	                     (if (n > 0 & n = system.Core/step!)
	                         (put(Core/trace!, system, 0),
	                          put(Kernel/debug!, system, -1),
	                          printf("  `GREENread: ~S(~S) = ~S\n", p, x, z),
	                          put(Kernel/debug!, system, d),
	                          put(Core/trace!, system, n)),
	                    Core/set_index(back_index),
						system.Kernel/debug! := back_debug,  
						z)
...
self_print(self:Assign) : void
 -> (let a := self.arg,
       ...
       (if sugar?(self.var, {}, o, (case a (Call a.args[1])))
        ...
        else printf("~S `BLUE:= `BLACK~I~I", self.var, lbreak(2), printexp(a, true)),
        pretty.index :- 2))
...
self_print(self:Gassign) : void
 -> (let a := get(arg, self),
       ...
       (if sugar?(self.var, {}, o, (case a (Call a.args[1])))
        ...
        else printf("~S `BLUE:= `BLACK~I~S", self.var, lbreak(2), a),
        pretty.index :- 2))
...
self_eval(self:Call_method) : any
  -> (if (system.Kernel/debug! != -1) mClaire/push!(self),
  	let start := mClaire/index!(), Cprop := self.arg in
        (for x in self.args mClaire/push!(eval(x)),
         let r := execute(Cprop, start, true)
         in (if (system.Kernel/debug! != -1) mClaire/set_index(start - 1),
         	r)))
...
self_print(x:Cast) : void
 -> printf("~I `BLUEas `BLACK~I", printexp(x.arg, false), printexp(x.set_arg, false))
...
self_print(self:Return) : void
 -> printf("`BLUEbreak`BLACK(~I~S~I)", (pretty.index :+ 2), self.arg,
           (pretty.index :- 2))
...
[substitution(self:any,x:Variable,val:any) : any
 -> case self
      (Variable (if (self.mClaire/pname = x.mClaire/pname) val else self),
		thing (if (self.name = x.mClaire/pname) val else self),
       bag (for i in (1 .. length(self))
              (if (self[i] % Variable | self[i] % unbound_symbol | self[i] % thing )
       ...
       Instruction (for s in owner(self).slots
                      let y := get(s, self) in
                         (if (y % Variable | y % unbound_symbol | y % thing)
