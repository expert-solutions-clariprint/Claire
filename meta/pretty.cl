...
DEBUG?:boolean := false
CAN_INIT_LOCATION?:boolean := false
//<sb> map locations of instructions
iClaire/CODE_LOCS[e:any] : list := nil
iClaire/equip :: property(open = 3)
init_location :: property(open = 3)
[close(self:Instruction) : Instruction ->
	if CAN_INIT_LOCATION?
		init_location(self),
	self]

[option_respond(opt:{"-debug"}, l:list[string]) : void -> DEBUG? := true]
[option_usage(self:{"-debug"}) : tuple(string, string, string) ->
	tuple("Debugger", "-debug",
...
Variable[mClaire/pname,range] <: Basic_instruction(
	...
	iClaire/dindex:integer = 0)            //<sb> tell that the variable is used for debug



self_print(self:Variable) : void ->
  (when s := get(mClaire/pname,self) in princ(s) else princ("<unamed variable?>"))


ppvariable(self:Variable) : void
 -> (if known?(range, self)
        printf("~A:~I", self.mClaire/pname, printexp(self.range, false))
     else printf("~A", self.mClaire/pname))
...
lexical_build(self:any,lvar:list,n:integer) : any ->
	let x := lexical_build_imp(self, lvar, n)
	in (if DEBUG?
			(//if CALL_ON_LEXICAL_BUILT?
			//	apply(on_lexical_built, list(x)),
			x := equip(x)),
		x)

lexical_build_imp(self:any,lvar:list,n:integer) : any
 -> (if (self % thing | self % unbound_symbol) lexical_change(self, lvar)
            ...
            (Variable (if unknown?(index,self)                          // v3.1.12
             ...
             Call let s := lexical_change(self.selector, lvar) in
                    (lexical_build_imp(self.args, lvar, n),
                     if (self.selector != s)
                            ...
                            for s in %type.slots
                              let x := get(s, self) in
                                (if ((x % thing | x % unbound_symbol) &
                                 ...
                                 else lexical_build_imp(x, lvar, n))),
