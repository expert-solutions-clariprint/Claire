	...
	in (if (system.Kernel/debug! != -1)
		...
		let %c := self.arg,
	       ...
	       (if (%c.open != ephemeral()) write(instances, %c, %c.instances add %o),
	        let res := complete(%o, self.args)
			in (Core/set_index(back_index),
...
claire/reified? :: property(open = 3)
reified?(self:type) : boolean -> false
     ...
     let p := make_a_property(self.arg.selector),
       ...
       (if (p.open > 0 & p.open <= 1 & p.mClaire/dispatcher = 0)       // v3.2.22 : no warning for interface methods                                               // v3.2.04
         when r := some(r in (p.restrictions but m) | (r.domain ^ m.domain)) in
           ...
           trace(1,"`BLUE--- WARNING ! [186] conflict between ~S and ~S is dangerous since ~S is closed\n", m,r,p), // v3.2.06
        LDEF := list<any>(),
        if (lbody[3] != body) put(formula, m, lambda!(lv, lbody[3])),
        if (length(lrange) > 1) put(typing, m, lrange[2]),
        write(inline?, m, self.inline?),
		when bd := get(formula, m)
		in let tp := tuple(m.module!, m.selector)
			in (CODE_LOCS[tp] := CODE_LOCS[self.arg],
	    		fill_debug_placeholder(bd.body, tp)),
        attach_comment(m),
        if (p = close & not(m.range <= domain!(m)))                     // v3.2.01
          error("[184] the close method ~S has a wrong range",m),
        m))

[fill_debug_placeholder(x:any, m:any) : void ->
	if Language/DEBUG?
		(//[4] fill_debug_placeholder(~S, ~S) // x, m,
		case x
			(Do (if (length(x.args) > 0 & x.args[1] % Call &
								x.args[1].selector = Core/push_frame)
					let c := x.args[1]
					in (if (c.args = list("placeholder"))
							let l := list<any>(),
								loc := CODE_LOCS[m],
								addloc? := true,
								i := 1,
								len := length(x.args)
							in (put(args,c,list(m)),
								l add c,
								while (i <= len)
									let y := x.args[i]
									in (if (y % Call & y.selector = Core/push_arg)
											l add y
										else if (loc & addloc?)
											(l add Call(Core/update_location, loc),
											addloc? := false)
										else l add y,
										i :+ 1),
								put(args, x, l))))))]
...
(let e1 := entries.restrictions[1],
	e2 := entries.restrictions[2]
in (put(range, e1, list[string]),
	put(range, e2, list[string])))

(let e1 := explode.restrictions[1],
	e2 := explode.restrictions[2]
in (put(range, e1, list[string]),
	put(range, e2, tuple(integer, (1 .. 12), (1 .. 365), (1 .. 31), (1 .. 7),
									(0 .. 23), (0 .. 59), (0 .. 59), boolean ))))

(put(range, explode_wildcard.restrictions[1], list[string]))
