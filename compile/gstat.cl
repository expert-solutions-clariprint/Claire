...
[statement(self:any,s:any,loop:any) : void
 -> let b := OPT.alloc_stack, ct := c_type(self), ps := psort(ct) in
      (if not(self % ((Tuple U Do) U Let)) OPT.alloc_stack := false,   // v3.2.26
;       printf("/* <~S> s=~S c_type=~A psort=~S */~I", owner(self), s,
;			(print_in_string(),
;			print(ct),
;			replace(end_of_string(), "*/", "* /")),
;			(print_in_string(),
;			print(ps),
;			replace(end_of_string(), "*/", "* /")),
;			breakline()),
	   if c_func(self)
         (if (s % string &
			...
			printf("~I = ~I~I;~I", c_princ(s),                           // v3.1.12 -> use bool_exp!
					(case self //<sb> handle case where the Optimizer loose a cast
						(Call_method
							(if (ps != void & ps != self.arg.range)
								printf("(~I)", interface!(ps))),
						to_C
							(if (ps != void & ps != self.set_arg)
								self.set_arg := ps))), //*/ none),
                  (if bool_exp?(self) bool_exp!(PRODUCER,self,loop)
...
[self_statement(self:Return,s:any,loop:any) : void
 -> if (loop = unknown)
        ...
        // breakline(),
         if OPT.loop_gc
          princ("GC_UNLOOP;"),
         princ("ClEnv->cHandle = loop_handle; break;"),
         close_block())
     else if (loop % string)
        (new_block(),
        if OPT.loop_gc
          princ("GC_UNLOOP;"),
         printf("~IClEnv->cHandle = loop_handle;break;", statement(self.arg, loop, {})),
         close_block())
...
[self_statement(self:Call,s:any,loop:any) : void
 -> let l := args(self), ld := unfold_args(l),
 		c := Call(selector = selector(self))
 	in (c.args := list{unfold_arg(l,ld,z) | z in l},
      		unfold_use(ld, c, s, loop))]


// A call method is now simpler with unfolding !
[self_statement(self:Call_method,s:any,loop:any) : void
 -> let l := args(self), ld := unfold_args(l),
 		c := Call_method(arg = arg(self))
 	in (c.args := list{unfold_arg(l,ld,z) | z in l},
      		unfold_use(ld, c, s, loop))]

[self_statement(self:Call_method1,s:any,loop:any) : void
 -> let l := args(self), ld := unfold_args(l),
 		c := Call_method1(arg = arg(self))
 	in (c.args := list{unfold_arg(l,ld,z) | z in l},
      		unfold_use(ld, c, s, loop))]

[self_statement(self:Call_method2,s:any,loop:any) : void
 -> let l := args(self), ld := unfold_args(l),
 		c := Call_method2(arg = arg(self))
 	in (c.args := list{unfold_arg(l,ld,z) | z in l},
      		unfold_use(ld, c, s, loop))]
