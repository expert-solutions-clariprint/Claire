...
[c_code_call(self:Call,sx:class) : any
 -> //[5] c_code_call(~S)  // self,
 	let  s := self.selector, l := self.args  in
	  (if (self.selector = Core/update_location & self.args & self.args[1] = *debug_local_file*)
			self.args[1] := Call(externC, list((print_in_string(),
			                                    printf("~S", *debug_local_file*),
				...
				(slot let %unknown :=  (not(get(default, z) % z.range) &
						...
						(if (not(%unknown) | designated?(l[1])) // v3.1.04
							Call_slot(selector = z,
									  ...
									  test = %unknown)
						 else (Optimize/show_opportunist(3),
							  //[3]  `GREEN note: ... unsafe access to unknown slot: ~S // self,
							  if compiler.optimize? (notice(), trace(3,"`GREEN poorly optimized slot access: ~S\n", self)),  // v3.3
							  c_warn(s,l,%type))),
				 method (//[5] found a method //,
						 if (s = nth=) OPT.use_nth= := true,
						 if (void % %type)
							Cerror("[205] call ~S uses a void argument [~S]",self,%type), // v3.3
...
[daccess(self:any,b:boolean) : any
 -> case self
      (Call let l := self.args,
              ...
              (if Language/DEBUG? unknown //<sb> avoid daccess when debug (would produce _integer_(xx->aa))
				else if (case xs
        ...
        if (p % OPT.to_remove) nil
           ...
           (if (y != unknown & not(yt ^ srange(s)))
               (//[5] ====> ~S ^ ~S = ~S // yt, srange(s), yt ^ srange(s),
                warn(),trace(2,"`BLUE sort error in ~S: ~S is a ~S [253]\n",self,y,yt)),
            if ((yt <= s.range | yt <= object | srange(s) != object | y = unknown) &   // v2.4.9 protect put
              ...
              c_code(Call(store, list(x,s.index,srange(s),y,p.Kernel/store?)))
            else (if compiler.diet? (warn(), trace(2,"`BLUE ~S is not a diet call [254]",self)),
                  if (compiler.optimize? & p != instances)
                     (notice(), trace(3,"`GREEN poorly typed update: ~S\n", self)),  // v3.3
               ...
               (Optimize/show_opportunist(3),
				//[3]  `GREEN note: ~S is poorly typed (~S,~S) // self, c_type(x), yt,
                case z
               ...
               let x2 := c_code(x, psort(domain!(s))) in
                  c_code(Call(add,
                              list(Call_slot(selector = s, arg = x2,
                                             test = false), y)))
            else (if compiler.optimize? (notice(), trace(3,"`GREEN poorly typed update: ~S\n", self)),  // v3.3
 ...
 -> let %t1 := c_type(self.args[1]),  // known to be a subset of bag
     ...
     (//[5] ~S: ~S -> ~S // self, %ltype, z,
      if (not(%t2 <= member(%t1)) & self.selector = add)
         (warn(),trace(2,"`BLUE the bag addition ~S is poorly typed (~S) [251] \n",self,member(%t1))),  // v3.3
      case z
 ...
 -> let l := self.args,
       ...
       (if (x % OPT.to_remove) nil
           ...
           (if (compiler.optimize? & (t <= array | t <= table))
               (notice(), trace(3,"`GREEN poorly typed call: ~S\n", self)),  // v3.3
 ...
 -> let sp := self.selector,
       ...
       (if (p % OPT.to_remove) nil
           ...
           (if (Update?(p, x, y) & (p.params % list | p.params % integer))
               ...
               (if compiler.optimize? (notice(), trace(3,"`GREEN poorly typed update: ~S\n", self)),  // v3.3
...
[nth_type_check(tl:type,ti:type,tx:type) : any
  =>  if not(tx <= member(tl))
         (warn(), trace(2,"`BLUE unsafe update on bag: type ~S into ~S [252]\n", tx,tl)),
      tx ]
...
[c_inline?(self:method,l:list) : boolean
 -> if (Language/DEBUG? & not(self.module! % {Core,Language,Reader})) false //<sb> avoid inlining in debug
	else let f := self.formula, la := f.vars, x := f.body, n := 1 in
       not((for v in f.vars
