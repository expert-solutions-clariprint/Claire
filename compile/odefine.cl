       ...
       (if known?(of,self)
          (if (compiler.safety > 4 | self.of = {} |
              ...
              (x.of := self.of, x)
                 ...
                 trace(2,"`BLUE unsafe typed list: ~S not in ~S [262]\n",
        ...
        (if known?(of,self)
          (if (compiler.safety > 4 | self.of = {} |
              ...
              (x.of := self.of, x)     // the set expression is type safe
                 ...
                 trace(2,"`BLUE unsafe typed set: ~S not in ~S [262]\n",
                       list{c_type(%x) | %x in self.args}, self.of),
...
[analyze!(c:class,self:any,%l:list,lp:list) : any
 -> let ins?:boolean := (c.open != ephemeral() & not(lp) & not(compiler.class2file?)), // PATCH!
        r := list<any>{ (if not(x.args[1] % property)
                          error("[internal] Wrong instantiation body for ~S, ~S is not a property", c, x.args[1]),
                        let p := x.args[1], y := x.args[2],
...
c_code(self:Defobj,s:class) : any
 -> let %a := OPT.allocation,
       ...
       (//[5] compile defobj(~S) -> ~S // self, o,
        ...
        else (warn(),trace(2, "`BLUE ~S is unknown [265]\n", self.Language/ident)),
        %x := c_code(%x, s),
     ...
     (lbody[2] := get(functional,m),
      if (not(compiler.inline?) & (px = Iterate  | px = iterate)) nil
             ...
             let na := function_name(px, ls[2], lbody[2]),
				...
				tp := tuple(m.module!, m.selector) in
             (Language/CODE_LOCS[tp] := Language/CODE_LOCS[self.arg],
			 if Language/DEBUG?
				Language/fill_debug_placeholder(la.body, tp),
...
[sort_code(self:Defmethod, lv:list) : any
  -> let l := lv[1], f := self.body.args[1].args[1],           // sort_pattern(self) = true !
       ...
       (Optimize/show_opportunist(2),
		//[2]  `GREEN note: quick sort optimisation for ~S ---- // self.arg.selector,
        ...
        if known?(f)
           (x := body,
            if (length(f.args) > 1)(try s := integer!({eval(u) | u in cdr(f.args)}
                                               ...
                                               SHIT := cdr(f.args),
                                               trace(2,"`BLUE wrong status ~S -> ~S [266]\n",
                                                        f,set!(cdr(f.args))),
                                               s := 0))
            else s := 0,
...
[function_name(p:property,l:list,x:any) : string
 -> if (x % function) string!(x as function)
            ...
            (if (compiler.naming = 0 & p != main)     // v3.1.04
             ...
             r := (if (p = main) r //<sb> check for main
             		else if (n <= 1) r /+ "1" else r /+ string!(m)), //<sb> append 1 to a function name (compilation multi module)
