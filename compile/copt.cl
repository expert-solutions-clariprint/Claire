...
[inline_exp(c:c_producer,self:Call_method1,loop:any) : void
 -> let m := self.arg, p := m.selector, a1 := car(self.args) in
       (if (p = - & (domain!(m) = integer | domain!(m) = float))
        ...
        else if (m = *length_string*)
           printf("LENGTH_STRING(~I)", expression(a1, loop))
 ...
 -> let m := self.arg, p := m.selector, a1 := self.args[1], a2 := self.args[2] in
      (//[4] inline_exp2: a1 = ~S, a2 = ~S // a1, a2,
		if (m = *min_integer* & Compile/designated?(a1) &
           ...
           printf("_char_(~I[(CL_INT)(~I) - 1])", expression(a1, loop), expression(a2, loop))
        else if (m = *+_char**)
			printf("(~I + ~I)", expression(a1, loop), expression(a2, loop))
           ...
           printf("~I(~I~I~A~I~I)",     //<sb> add CL_INT check
                  (if (compiler.safety < 2 & c_type(a1) <= integer) princ("_integer_")),
           		  (if (c_type(a1) <= integer) princ("(CL_INT)")),
                  bexpression(a1, loop),        // v3.2.06
                  string!(p.name),
           		  (if (c_type(a2) <= integer) princ("(CL_INT)")),
                  bexpression(a2, loop))
           ...
           printf("~I->addFast((OID)~I)", expression(a1, loop), expression(a2, loop))
        ...
        else if (m.selector = externC)
        	printf("(~I)(~I)", interface!(PRODUCER,a2), princ(a1))
...
[inline_exp(c:c_producer,self:Call_method,loop:any) : void
 -> let m := self.arg, b := OPT.Compile/alloc_stack,
       ...
       (OPT.Compile/alloc_stack := false,
        //[4] inline_exp: a1 = ~S, a2 = ~S, a3 = ~S // a1, a2, a3,
        if (m = *nth=_list* & compiler.safety >= 3)
           printf("((*(~I))[~I]=~I)", expression(a1, loop),
                  expression(a2, loop), expression(a3, loop))
                                         ...
                                         m.selector = nth_put))
           printf("(~I[~I - 1] = (unsigned char) ~I->ascii)", expression(a1, loop),
                  expression(a2, loop),
                  expression(a3, loop))
           ...
           printf("STOREI(~I /* 121 */,(OID*)&((*~I)[~I]),(CL_INT)~I)",
                   bexpression(a1,loop), // expression(getC(a1), loop), // 
                   expression(a1, loop),
                   expression(a2, loop),
                   expression(a3, loop))
                 ...
                 ((length(self.args) = 4 & self.args[4] = true) | length(self.args) = 3))
//           printf("STOREI((ClaireObject*)~I /* 129 */,(int*)&((ClaireArray*)(*~I))[~I]),(CL_INT)~I)",
           printf("STOREI((ClaireObject*)~I - 2 /* 129 */,(OID*)(~I + ~I),(CL_INT)~I)",
                        expression(getC(a1), a1),
                        expression(a1, loop),
                        expression(a2, loop),
                        expression(a3, loop))
...
//                            (princ(","), expression(self.args[6],loop)))  )
                            (princ(","), breakline(), expression(make_function(string!(self.args[5]) /+ "_"),loop)))) //<sb> strange
...
[print_external_call(c:c_producer,self:Call_method,loop:any) : void
 -> let m := self.arg, l := self.args, n := 1, ld := m.domain,
 		d? := (m.module! % compiler.debug?) in
...
[inline_exp(c:c_producer,self:Call,loop:any) : void
 -> let  p := self.selector, a1 := car(self.args), n := length(self.args) in
       (if (p = mClaire/get_stack) printf("ClEnv->stack[~I]", expression(a1, loop))
        ...
        else if (p = mClaire/set_index & length(self.args) = 1)
          ...
          let a2 := self.args[2] in     // a class
              printf("(~I.~I = ~I)",
			   ...
			   (if (self.selector = store)
				...
				else error("[216] ~S has more than 20 parameters",self))
  ...
  -> let p := x.selector, a1 := x.args[1],
      ...
      ( (designated?(a1) | p.mClaire/dispatcher > 0) &  // new v3.2.26: even simpler
          ...
          (if (FCALLSTINKS & not(selOK & lrOK & callOK) & p.mClaire/dispatcher > 0)
              (trace(1,"**** fcall(~S) fails : selectorOK = ~S, lrOK = ~S, callOK = ~S\n",
                                  x, selOK, lrOK, callOK)),
           (selOK & lrOK & callOK)))) ]
  ...
  -> let p := x.selector, a1 := x.args[1],             // was (x.args[1] as to_CL).arg,
       ...
       (if (s = void) printf("_void_(")
        ...
        if (p.mClaire/dispatcher > 0 & length(lt) <= 4)    // use fcall
          (Optimize/show_opportunist(4),
			//[4]  `GREEN note: ~S is compiled with fast dispatch // x,
           if (s = object) princ("(ClaireObject *) "),
           printf("~I->fcall(~I)", expression(p,loop),
                                  c_sorted_args(x,m.mClaire/srange,loop,true)))  //<sb> fix the tuple stack case
        else let lr2 :=  list{x in p.mClaire/definition | x % lr} in  // good order
             (Optimize/show_opportunist(4),
			  //[4]  `GREEN note: ~S is compiled with explicit dispatch (s = ~S) // x, s,
              for m in lr2
;               let psort := list{ (let c := class!(x), sc := sort!(c) in    // sort! = osort :-)
;                                    (if (sc = object) c else sc)) |
;                                  x in m.domain} in
               let psort := list{ psort(x) | x in m.domain} in
               (if (m != last(lr2))
                   printf("(INHERIT(~I,~I) ?~I ~I ~I((~I *) ~I)~I : ~I ",
                       ...
                       (if (m.range % tuple & not(OPT.Compile/alloc_stack)) princ("->copyIfNeeded()")), //<sb> fix the tuple stack case
                       breakline())
...
[signature!(c:c_producer,x:any) : void
 -> case x (list printf("list::domain(~A,~I)", length(x),
                        args_list(list{ /*getC*/ Optimize/c_gc!(getC(y)) | y in x}, nil, length(x) > 3)),
            to_C signature!(c,x.arg),
            to_protect signature!(c,x.arg),
            List signature!(c,x.args),
            Variable expression(x,nil),
            any printf("<fucking ~S:~S>",owner(x),x)), setenv("CLDEBUG=0") ]
...
[gassign(c:c_producer,self:Gassign,loop:any) : void
 -> if self.var.Kernel/store?                                     // still ok in v3.3 since storeable vars use the old pattern
        printf("(STOREI(~I /*  404 */,(OID*)&(~I),(CL_INT)~I))",
               expression(self.var, self.var),
			         expression(self.var, loop),     //!
               expression(get(arg, self), loop))
    else printf("(~I= ~I)", expression(self.var, loop),
                expression(get(arg, self), loop)) ]

[call_slot(c:c_producer,self:Call_slot,loop:any) : void
 -> let r := some(r in preread.restrictions |
						r != preread.restrictions[1] &
						Core/tmatch?(list(owner(self.selector.selector), c_type(self.arg)),r.domain))
	in (if known?(r)
			printf("(~I,", expression(c_code(Call(preread,list(self.selector.selector, self.arg))), loop)),
		if self.test
		       printf("~A(~I,",
...
[update(c:c_producer,self:Update,loop:any) : void
 ->  let p:any := self.selector, a := self.arg,
       ...
       (if (case p (relation (known?(if_write,p) & a != put & a != put_store)))
           printf("~I_write(~I,~I)",
                  ...
                  (if (s = any | s = integer) 
                    printf("STOREI(~I /* 464 */,(CL_INT*)&(~I),(CL_INT)~I)",expression(x.arg,loop),expression(x, loop),expression(v, loop))
                   else if (s = float)
                    printf("STOREF(~I,(double*)&(~I),~I)",expression(x.arg, loop),expression(x, loop),expression(v, loop))
                   else printf("STOREO((ClaireObject*)~I,(ClaireObject**)&(~I),~I)",expression(x.arg, loop),expression(x, loop),expression(v, loop)))
        else printf("_void_(~I = ~I)", expression(x, loop), expression(v, loop))) ]
...
[exp_Assign(c:c_producer,self:Assign, loop:any) : void
  -> let x := self.arg, v := self.var
  	in printf("~I= ~I(~I)",
				ident(PRODUCER, v),
				(if (get(range,v) = integer)
					princ("(CL_INT)")),
				expression(x, loop))]   // v3.0.3 AUDACIEUX
...
[stat_handle(c:c_producer,self:Handle,s:any,loop:any) : void
 ->  new_block(),
     ...
     princ("ERROR_FREE;"), //<sb> use C++ macro was ClEnv->cHandle--;"),
     close_block(),
...
[stat_while(c:c_producer,self:While,s:any,loop:any) : void
 -> new_block(),
    let f? := (c_func(self.test) & not(self.other) & gc_usage(self.test,loop) = false),
        v := check_var("v_while", s, loop),
        oldgcloop := OPT.loop_gc,
        bloop := (OPT.loop_gc & gc_usage(self.arg,true) != false) in
       (printf("int loop_handle = ClEnv->cHandle;"), //<sb> support for "while (...) try ... break"
       breakline(),
        OPT.loop_gc := bloop,
        if bloop printf("OID gc_local;~I", breakline()),
...
[stat_gassign(c:c_producer,self:Gassign,s:any,loop:any) : void
 ->  new_block(),
     ...
     breakline(),
     if self.var.Kernel/store?  printf("STOREI(~I /* 590 */ ,(CL_INT*)&(~I),truc);",expression(self.var, loop),expression(self.var, loop))
...
[stat_for(c:c_producer,self:For,s:any,loop:any) : void
 -> let v := c_string(c,self.var),
      ...
      (new_block(),
       ...
       if (s % string) (breakline(), printf("~I= Kernel.cfalse;", c_princ(s))),
       breakline(),
       ...
       if bloop (printf("GC_UNLOOP; POP_SIGNAL;"), close_block()),
 ...
 -> if not(s) error("[203] you should have used a FOR here:~S", self),
    let v := c_string(c,self.var),
      ...
      (new_block(),
       ...
       printf("OID ~I; CL_INT CLcount;~I", c_princ(v), breakline()),
       printf("~I ~I = ~I;",
       ...
       if bloop printf("GC_UNLOOP; POP_SIGNAL;"),
       OPT.loop_gc := oldgcloop,
       close_block(),
       close_block()) ]
...
/*[stat_let(c:c_producer,self:Let,s:any,loop:any) : void
 -> let ns := c_string(PRODUCER,self.var.pname) in
     ...
     let v := c_string(PRODUCER,self.var), b := OPT.alloc_stack,
         x := get(value, self), f := true, inLet*?:any := false in   // f is used for recursively embedded lets
       (new_block(),
           ...
           if c_func(x)
            (printf(" = ~I;~I",
               (if bool_exp?(x) bool_exp!(PRODUCER,x,loop) else expression(x, loop)),   // v3.1.12
                   breakline()),
            if (self % Let* & sort(self.var) Core/<=t tuple)
            	(if OPT.protection
            		printf("GC_PUSH(~I);~I", c_princ(v), breakline()),
            	inLet*? := v)
            else if inLet*?
            	(if (OPT.protection & x % Call_method2 & x.arg = (nth @ bag) & c_string(PRODUCER,x.args[1]) = inLet*?)
            		printf("GC_OID(~I);~I", c_princ(v), breakline()))
            )
