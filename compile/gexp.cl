...
// ---------------------------------------------------------------------
// Contents
//   Part 1: when to apply : c_fonc
//   Part 2: expression for objects
//   Part 3: expression for messages
...
expression(self:integer,loop:any) : void -> printf("~A", self)
...
[bexpression(self:any,loop:any) : void
  -> case self (Assign printf("(~I)",expression(self,loop)),
                ...
                integer (if (self < 0)  printf("((CL_INT)~I)",expression(self,loop))    // v3.2.44
...
[expression(self:Assign,loop:any) : void
 -> ///[-1] expression(~S,~S), loop_gc = ~S // self, loop, OPT.loop_gc,
 let x := self.arg, v := self.var
 in	(if (case x (Call_method2
...
[bool_exp(self:Call_method2,pos?:boolean,loop:any) : void
 -> let m := self.arg, p := m.selector, lop := PRODUCER.open_comparators,
      ...
      (if (p = !=) equal_exp(PRODUCER,a1, not(pos?), a2, nil)
           ...
           printf("(~I~I ~I ~I~I)", //<sb> add CL_INT check
                  (if (domain!(m) = integer) princ("(CL_INT)")),
                  expression(a1, loop),
                  (if pos? print(p)
