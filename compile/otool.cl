...
self_print(self:Compile/to_C) : void -> printf("C{~S}:~S", get(arg,self),get(set_arg,self))
...
Compile/c_gc?(self:Compile/to_C) : boolean
  -> (not(gcsafe?(self.set_arg)) & Compile/c_gc?(self.arg) &
      ...
      self.set_arg Core/<=t primitive)) 	//<sb> add primitive check                                  
...
self_print(self:Compile/C_cast) : void
 -> printf("<~S:~S>", get(arg,self),get(set_arg,self))
...
self_print(self:Pattern) : void
 -> printf("~S[tuple(~A)]", get(selector,self), get(arg,self))
...
//<sb> show the location of the method currently compiled that
// issue warning or notes
*oportunist_method*:any := unknown

show_opportunist(n:integer) : void ->
	(if (verbose() >= n &
			(case get(in_method, OPT)
				(method OPT.in_method != *oportunist_method*)))
		(*oportunist_method* := OPT.in_method,
		when loc := Reader/CODE_LOCS[OPT.in_method]
		in (if loc
				(if (color() = 2)
					//<sb> TODO: make a single method with file.cl
					let href := getenv("WCL_HREF_FORMATER"),
						p := buffer!(ctrace(), 4096),
						old := use_as_output(p)
					in (color(0), //<sb> we have to set the no-color mode to avoid convertions
						...
						?>&nbsp;<a href='<?= href ?>'><?== loc[1] ?>:<?= loc[2] ?></a>&nbsp;<?
						use_as_output(old),
						fclose(p),
						color(2))
				else printf(ctrace(), "~A:~S ", loc[1], loc[2]))),
		if (color() = 2)
			let href := getenv("WCL_HREF_FORMATER"),
				p := buffer!(ctrace(), 4096),
				old := use_as_output(p)
			in (color(0),
				...
				fclose(p),
				color(2))
		else printf(ctrace(), "[~S]\n", OPT.in_method)))

// unified warning
Compile/warn()  : void
 -> (show_opportunist(2),
	trace(2," `BLUE warning: "))

Compile/Cerror(s:string,l:listargs) : {}
  -> (show_opportunist(-1000),
	general_error(Core/cause = "---- Compiler Error :\n" /+ s, arg = l))
...
[c_warn(self:Call,%type:any) : any
 -> let s := self.selector in
       (if (%type = void)  Cerror("[205] message ~S sent to void object", self)
           ...
           (warn(),trace(2,"`BLUE the property ~S is undefined [255]\n", s))
        else if (not(s % OPT.ignore) & (s.open <= 1 | s.open = 4) &
                 (case %type (list class!(%type[1]).open != 3)))
            (warn(), trace(2,"`BLUE wrongly typed message ~S (~S) [256]\n", self, %type))
        else if compiler.optimize?
            (notice(), trace(3,"`GREEN poorly typed message ~S [~S]\n", self, %type)),   // v3.3 poor opt. notice
 ...
 -> let s := self.selector in
       (if (%type = void)  Cerror("[205] message ~S sent to void object", self)
           ...
           (Optimize/show_opportunist(4),
			trace(3,"  `GREEN note: wrongly typed message ~S [~S]\n", self, %type)),
...
[c_warn(self:property,l:list,%type:list) : any
 -> if (self.open <= 1 & not(self % OPT.ignore) &  compiler.safety > 1)
        (Optimize/show_opportunist(4),
		trace(4, "`GREEN note: poor type matching with ~S(~S) [~S]\n", self, l, %type)),
    open_message(self, l) ]
...
[c_warn(self:Variable,x:any,y:type) : any
 -> if not(y ^ self.range)
        (if (compiler.safety > 4)
            (warn(), trace(2,"`BLUE ~S of type ~S is put in the variable ~S:~S [257]\n",
                            x, y, self, self.range))
...
[enumerate_code(self:any,%t:type) : any
 -> if (ptype(%t) <= bag) c_strict_code(self, bag)              // v3.2.01
    else (if compiler.optimize? (notice(), trace(3,"`GREEN explicit enmeration of ~S\n", self)),  // v3.3
...
[range_infers_for(self:Variable,y:type,ts:type) : any
 ->  if unknown?(range, self)
        ...
        (if not((y & self.range))
            (warn(), trace(2, "`BLUE range of variable in ~S is wrong [258]\n", self))),
...
[range_sets(self:any,y:type) : void
 -> case self
      (Variable (//<sb> fix range if needed
				let rg := (if known?(range, self) self.range else any)
				in (if sort=(osort(rg), osort(y))
	                   let c1 := psort(class!(rg)) in
	                     (if (c1 != psort(class!(y)))
	                         put(range, self,
	                             Union(Core/t1 = {},
	                                   Core/t2 =  Union(Core/t1 = c1, Core/t2 = y)))
	                      else put(range, self, y))
	                 else if (osort(rg) = any)
	                   put(range, self, sort_abstract!(y))))) ]
...
[Compile/return_type(self:any) : type
 -> case self
      (to_C return_type(self.arg),
       ...
       Handle return_type(self.arg), //<sb> QY: why not : ^ return_type(self.other)
       any {}) ]
...
/*c_code(self:Type,s:class) : any ->
	(OPT.protection := true,
	to_protect(arg = c_code(self_code(self), s)))*/
...
[gcsafe?(self:class) : boolean
 -> (self = void | (self inherit? thing & not(self inherit? table)) | self inherit? class |
     (self != object & self inherit? object & self.open < 3 & not(self inherit? collection) &
      self != lambda ) | //<sb> tables are not safe
...
[c_gc!(self:any) : any
 -> if (not(OPT.online?) & c_gc?(self))
        (Optimize/show_opportunist(4),
		//[4]  `GREEN note: use GC protection on ~S // self,
         OPT.protection := true,
         to_protect(arg = self))
     else self ]
...
[c_gc!(self:any,t:type) : any
 -> if (not(OPT.online?) & (c_gc?(self) |
         ...
         & not(gcsafe?(t)))
        (Optimize/show_opportunist(4),
		//[4] `GREEN note: use GC protection on ~S:~S // self, t,
       ...
       (x := Language/instruction_copy(x),
        ...
        for v in lbv
          let v2 := Variable(pname = (if (v.pname = pv0) pv0
                             ...
                             Kernel/index = 100000) in      // force to be a local !
       ...
       Call (if (self.selector = eval)                     // two patterns eval(e) or eval(e,R)
               ...
               try apply(self.selector,
                 ...
                 (//[0] a strange problem happens ~A // verbose(),
                  warn(),trace(2,"`BLUE failed substitution: ~S",system.exception!),
