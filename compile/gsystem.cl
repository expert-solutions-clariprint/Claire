...
Compile/*length_string* :: (length @ string)
Compile/*+_char** :: (+ @ char*)
Compile/*nth_char** :: (nth @ char*)
  ...
  -> when l := get(formula,m) in
        ...
        let s := void,
         ...
         (printf("Opt => ~S \n\n",%body),
          ...
          close_block()), //<sb> add the closing }
...
[claire/compile(self:module) : void
 -> OPT.need_modules := {},
 	PRODUCER.stat := 0, //<sb> focus on module GC stats 
      let l1:bag := parents(Reader/add_modules(list(self))),
          hs := compiler.headers
     in (compiler.headers :/+ list<string>{x in self.made_of|match_wildcard?(x,"*.h*")}, //<sb> add additional headers (support <...h> form)
       ...
       if compiler.class2file?
          (//[2] **** Compiling the module ~A [v. 3.~A - verbose:~A opt:~S] // string!(self.name), compiler.version, verbose(), compiler.optimize?,
           ...
           generate_c2f(self),
              ...
              if isfile?(compiler.source / external!(self) /+ PRODUCER.extension)
                unlink(compiler.source / external!(self) /+ PRODUCER.extension),
       ...
       //[1] ++++ v3.3.32 info: ~A GC protection inserted in ~S. // PRODUCER.stat, self,
       if l1 trace(2, "---- WARNING: ~S should be declared for ~S\n", l1, self)) ] //<sb> focus on module GC stats 
...
[generate_files(self:module) : void
 ->  //[0] ==== Generate ~A files for module ~S [verbose = ~A, Opt? = ~S] // PRODUCER.comment, self, verbose(),compiler.optimize?,
     ...
     begin(self),
     for x in list{x in entries(self.source)|match_wildcard?(x,"*.o") | match_wildcard?(x,"*.obj")} //<sb> unlink object files
		(if isfile?((self.source /+ *fs*) /+ x)
			(//[3] ++++ Unlink file ~A // x,
			unlink((self.source /+ *fs*) /+ x))),
     for x in list{x in entries(compiler.source)|match_wildcard?(x,"*.o") | match_wildcard?(x,"*.obj")} //<sb> unlink object files
		(if isfile?((compiler.source /+ *fs*) /+ x)
			(//[3] ++++ Unlink file ~A // x,
			unlink((compiler.source /+ *fs*) /+ x))),
	 for x in self.made_of
	  (if (match_wildcard?(x, "*.cl") | not(match_wildcard?(x, "*.*")))
		   (x := replace(x,".cl",""), //<sb> allow extension in made_of list
		   if (x = string!(self.name))
			   Cerror("[211] ~S cannot be used both as a file and module name",x),
			 generate_file((self.source /+ *fs*) /+ x, (compiler.source /+ *fs*) /+ external!(self)))
		else if match_wildcard?(x,"*.cpp")
		  (OPT.outfile := fopen(compiler.source / external!(self) /+ PRODUCER.extension, "a"),
		  start_file(compiler.source / external!(self), self),
		  let f := fopen(self.source / x, "r")
		  in (freadwrite(f, OPT.outfile),
		      fclose(f)),
		  fclose(OPT.outfile))),
    let p := fopen(compiler.source / external!(self) /+ PRODUCER.extension, "a"),
        old := use_as_output(p)
    in (create_dynload_module(self),
        use_as_output(old),
        fclose(p)),
     for x in list{x in self.made_of|match_wildcard?(x,"*@PATH_INFO@*")} //<sb> in xl version we can have wcl files
  		let t := explode(x, "@PATH_INFO@"),
  		...
  		in (if (left(fname,2) = "./") fname := substring(fname,3,length(fname)),
  			...
  			if (fname = string!(self.name))
  			   Cerror("[211] ~S cannot be used both as a file and module name", fname),
  			generate_wcl_file(t[1], x, compiler.source / external!(self))),
	 end(self) ]
...
[generate_file(s:string, m:module) : void
 -> start_file(s,m),
    ...
    breakline(),
    //printf("GC_UNBIND;"), //<sb> rm - protect optimized vars
    close_block(),
    breakline(),                       // v3.0.3
    fclose(OPT.outfile)]
...
[generate_c2f(self:module) : void
 -> let modulePort := fopen(compiler.source /+ *fs* /+ c_string(PRODUCER,self.name) /+ PRODUCER.extension,"a") in
      (OPT.outfile := modulePort,
       use_as_output(modulePort),
       generate_start_file(PRODUCER,self),
       putc("\n"[1],modulePort), //<sb> ensure new line at end of file - prevent C++ compiler warning
...
[generate_interface(self:module,l:bag) : void
 -> //[0] ==== Generate interface (.h) files for module ~S // self,
    ...
    putc("\n"[1],OPT.cinterface), //<sb> ensure new line at end of file - prevent C++ compiler warning
...
[generate_meta_load(self:module) : void   // v0.02
  -> printf("// definition of the meta-model for ~S~I",self, breakline()),
     ...
     let j:any := unknown, gcbind? := false in (
       for i in OPT.instructions (
          breakline(),
          if (i % string)  printf("~I/* ~A */", (if not(j % string) breakline()), i)
          else if global_var_def?(i)
          		(if gcbind?
          			(printf("GC_UNBIND;\n"), //<sb> introduce gc bind for temp objects
          			gcbind? := false),
          		global_var_def!(PRODUCER, self, i as Let)) // v3.3
          else
          		(if not(gcbind?)
          			(printf("GC_BIND;\n"),
          			gcbind? := true),
...
[generate_file(self:string,f2:string) : void
 -> let p1 := fopen_source(self),
 		f := Core/get_location(p1)[1],
 		b := reader.Reader/toplevel,
        p0 := reader.Reader/fromp
        in
       (//[2] ++++ Generate the file ~A [v3.~A - safety:~A] // f, compiler.version, compiler.safety,
       if not(compiler.class2file?)                 // v0.02
           OPT.outfile := fopen(f2 /+ PRODUCER.extension, "a"),
        ...
        if not(compiler.class2file?)
        	(start_file(self,module!()),		
        	if Language/DEBUG? //<sb> add the path of the source in debug
	          let p := use_as_output(OPT.outfile)
	          in (Optimize/*debug_local_file* := realpath(f),
                use_as_output(p))),
        let %instruction := Reader/readblock(p1) in
          //while not(%instruction = Reader/eof)
          while not(%instruction = EOF)    //<FXJ+NM> was: Reader/eof
            (if (system.verbose > 5)
       ...
       putc("\n"[1], OPT.outfile), //<sb> fix warning on some C++ compiler
       fclose(p1),
       compiler.loading? := false,
       reader.Reader/toplevel := b,
       reader.external := "toplevel",
       reader.Reader/fromp := p0,
	   Optimize/*debug_local_file* := "",
       if not(compiler.class2file?)
       		(putc("\n"[1],OPT.outfile), //<sb> ensure new line at end of file - prevent C++ compiler warning
       		fclose(OPT.outfile)))]

//<sb> special generate tool for WCL code
[generate_wcl_file(self:string, mof:string, f2:string) : void ->
	//[2] ++++ Generate the WCL file ~A [v3.~A - safety:~A] // self, compiler.version, compiler.safety,
	compiler.loading? := true,
	OPT.outfile := fopen(f2 /+ PRODUCER.extension, "a"),
	start_file(self, module!()),
	if Language/DEBUG?
	     let p := use_as_output(OPT.outfile)
	     in (Optimize/*debug_local_file* := realpath(self),
	        use_as_output(p)),
	let defs := Reader/load_wcl!(mof)
	in (for x in defs
			OPT.instructions :add c_code(x, void),
		putc("\n"[1], OPT.outfile), //<sb> fix warning on some C++ compiler
		compiler.loading? := false,
		if not(compiler.class2file?)
			(putc("\n"[1],OPT.outfile), //<sb> ensure new line at end of file - prevent C++ compiler warning
     ...
     let s := void,
	   ...
	   (generate_function_start(PRODUCER, self, s, m, %nom),        // remove one parameter (%f)
		...
		print_body(%body,%end,s,self.body,
				   not(OPT.protection | %profile | OPT.alloc_stack)),
		OPT.alloc_stack := false,
		printf(OPT.outfile, "~I\n\n", close_block()),
		use_as_output(stdout))]
...
[print_body(%body:any,%end:string,s:class,%original:any,%easy:boolean) : void
  -> //gc_introduction(PRODUCER,%body),               // v3.0.55
	let pfunc := blob!(),
     	oldout := OPT.outfile
	in (OPT.outfile := pfunc,
		use_as_output(pfunc),
		outer_statement(%body,%end,s,%easy),
		OPT.outfile := oldout,
		use_as_output(OPT.outfile),
		if (OPT.loop_index > 0) //<sb> optimal reserve
			printf("~IGC_RESERVE(~A);  // v3.3.39 optim !~I", breakline(), OPT.loop_index, breakline())
		else if OPT.protection printf("~IGC_BIND;~I",breakline(),breakline()),
		freadwrite(pfunc, oldout),
		fclose(pfunc),
		true)]

// special case when the body is a Do
[print_body(%body:Do,%end:string,s:class,%original:any,%easy:boolean) : any
 -> let l := %body.args, %length := length(l), m := 0,
 		pfunc := blob!(),
     	oldout := OPT.outfile
	in (//gc_introduction(PRODUCER,%body),        // v3.0.55
		OPT.outfile := pfunc,
		use_as_output(pfunc),
       for x in l
        (m :+ 1,
...
[check_sort(self:method) : class
 -> let s1 := self.range, b := PRODUCER.body,
	  ...
	  Optimize/show_opportunist(5),
       //[5]  `GREEN note: ~S's range was found to be ~S (vs. ~S) // self, s2, s1,
       if not(s2 <= s1)
         (Optimize/show_opportunist(2),
		 //[2]  `GREEN note: ~S's range was found to be ~S (vs. ~S) // self, s2, s1,
        ...
        let s3 := (s1 ^ s2) in
          (if (compiler.safety > 1 & osort(s3) = s)
              (PRODUCER.body := c_strict_check(PRODUCER.body, class!(s3)),
               ...
               nil)),
        if (s1 % tuple)
           (Optimize/show_opportunist(3),
		   //[3]  `GREEN note: ~S is a method with range <= tuple => use stack allocation // self,
	...
	if (OPT.allocation & not(status!(self)[SAFE_GC]))
           (//OPT.protection := true, // NOT NECESSARILY -> a verifier <yc> 7/98
            if not(status!(self)[NEW_ALLOC])
               (Optimize/show_opportunist(3),
				trace(3, "  `GREEN note: ~S may provoke an allocation \n", self)))
