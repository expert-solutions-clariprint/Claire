...
//<sb> installation directory (configure's prefix)
claire/claire_prefix:string :: "@CLPREFIX@"
claire/have_prefix?:boolean :: (claire_prefix != "NONE" & claire_prefix[1] != '@')
...
//   env = Id(compiler.env), //<sb> rm see bellow
   version = Id(version()),
   source = "",
   headers_dir = "",
...
(#if compiler.loading?
	(externC("\n#ifndef CLPC\n"),
	compiler.env := "@SYSARCH@", //<sb> configured under unix
	externC("\n#endif\n")))
...
claire/OPT :: meta_OPT(
    ...
    ignore = set(mClaire/index!, mClaire/set_index,
				object!, mClaire/base!, mClaire/set_base,
 ...
 -> let y := (case x (Call c_code_call(x,s), any c_code(x))),   // v2.4.9 safe sort for inline !
      ...
      (if (s = void | z = s | OPT.online?)
           (if (s = void & (case x (Call x.selector = =)))
               (warn(),
                trace(2,"  `BLUE note: Equality meant as an assignment: ~S [264]\n",x)),    // v3.3
...
// SLOT_UPDATE :     a slot is updated whose content is not gcsafe
