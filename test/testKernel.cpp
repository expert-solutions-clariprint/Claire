...
{int t1,t2;
  ...
  ClAlloc->logList = 20;
  ClAlloc->maxList = (1ul << 20);
  ClAlloc->maxSize = (1ul << 20) + (1 << 20);
  ClAlloc->maxGC = 100000;
  ClAlloc->maxStack = 20000;                   // <yc> 9/98 changed because of GC
  ClAlloc->maxHist = 10000;
  ClAlloc->maxEnv = 400;
  ClAlloc->maxHash = ((1ul << 11) + 200);       // <yc> 7/98 Changed for claire3 !
  ClAlloc->hashMask = ((1ul << 11) - 1);
...
{int *x;
  ...
  {printf("There is not enough memory for CLAIRE, try claire -s 0 0\n");
   for (n = 1; n < 20000000; n++ ) x = (CL_INT *) n;
   exit(0);}
  return x;}
...
void push_debug_property(property *p,int n, CL_INT m) {}
void pop_debug_property(property *p,int n,OID v) {}

// we make a simple search in the restriction list
ClaireObject* find_which_class(ClaireClass *c,list *l, CL_INT i, CL_INT j)
 {ITERATE(r);
...
{char *s1 = make_string_integer(10,ClRes->ascii['1']);
 ...
 check(nth_string(s2,2)->ascii == (CL_INT) 'e');
 check(strlen(s1) == 10);
